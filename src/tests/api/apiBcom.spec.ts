...
test('API-2, playbuzz', async ({ request }) => {
  ...
  expect(await response.json()).toHaveProperty('total_hits', 391038);

  expect(await response.json()).toHaveProperty('total_hits', 391038);
  expect(await response.json()).toHaveProperty('payload.currentItemCount', 25);
  expect(await response.json()).toHaveProperty('payload.totalItems', 391038);
...
test('AP-3, playbuzz id specific', async ({ request }) => {
  ...
  expect(await response.json()).toHaveProperty('total_hits', 1);

  expect(await response.json()).toHaveProperty('payload.currentItemCount', 1);

  const respBody = await response.json();

  expect(respBody.payload.items.find((item) => item.id === 'e2ba2133-70c4-4ea8-8c18-bf6f06c2c15a')).toBeTruthy();

  // const itemsId = respBody.payload.items.map((item) => item.id);
  // console.log(...itemsId);

  expect(respBody.payload.items.map((item) => item.id)).toContain('e2ba2133-70c4-4ea8-8c18-bf6f06c2c15a');
});
